...
--adresini, ölkəsini, sifarişi göndərən işçinin (employer) ad və soyadını 1 sütunda, yükü göndərənin(shipper) adı
--yükü göndərənin telefon nömrəsini select edin və ən yeni sifarişlərdən ən köhnəsinə görə sıralayın.
--1-ci yazilis
SELECT OrderId,CONVERT(varchar(12),OrderDate,107),CustomerName,Address,Country,CONCAT(FirstName,' ',LastName) AS FullName,ShipperName,Phone
FROM orders,customers,employees,shippers
ORDER BY OrderDate DESC

--2-ci Inner join ile yazmaq
SELECT OrderId,CONVERT(varchar(12),OrderDate,107) as 'New Format',CustomerName,Address,Country,CONCAT(FirstName,' ',LastName) AS FullName,ShipperName,Phone
FROM orders,customers,employees,shippers
SELECT * FROM orders
